...
class pycuda(ExtensionEasyBlock):
    ...
    def run(self):
        ...
        if not self.src:
            raise EasyBuildError("No source found for Python package %s, required for installation. (src: %s)",
                                 self.name, self.src)
        super(pycuda, self).run(unpack_src=True)
...
#        return super(pycuda, self).sanity_check_step(*args, **kwargs)

    def make_module_extra(self):
        """Add install path to PYTHONPATH"""

        txt = self.module_generator.prepend_paths("PYTHONPATH", [self.pylibdir])
        return super(pycuda, self).make_module_extra(txt)
