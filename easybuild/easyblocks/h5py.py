...
class h5py(ExtensionEasyBlock):
    ...
    def __init__(self, *args, **kwargs):
        ...
        super(h5py, self).__init__(*args, **kwargs)
	...
	cmd = "python setup.py configure --mpi"
    ...
    def run(self):
        ...
        if not self.src:
            raise EasyBuildError("No source found for Python package %s, required for installation. (src: %s)",
                                 self.name, self.src)
        super(h5py, self).run(unpack_src=True)
